...
2. 生产者状态管理：Kafka 生产者会在发送消息时维护一个内部状态，包括消息的序列号。每个消息都有一个唯一的序列号，Kafka 会使用该序列号来判断是否是重复的消息。
当消息发送到 Kafka Broker 时，Broker 会检查这个序列号，如果是新的序列号，则写入消息；如果是重复的序列号，则丢弃消息。
